...
import org.copticchurchlibrary.arabicreader.adapter.DBSlidingTripAdapter;
import org.copticchurchlibrary.arabicreader.dataMng.SoundCloundDataMng;
import org.copticchurchlibrary.arabicreader.dataMng.TotalDataManager;
import org.copticchurchlibrary.arabicreader.fragment.FragmentHome;
import org.copticchurchlibrary.arabicreader.fragment.FragmentPlaylist;
import org.copticchurchlibrary.arabicreader.object.DBImageLoader;
...
import org.copticchurchlibrary.arabicreader.setting.ISettingConstants;
import org.copticchurchlibrary.arabicreader.setting.SettingManager;
import org.copticchurchlibrary.arabicreader.view.PagerSlidingTabStrip;
import org.copticchurchlibrary.arabicreader.view.SliderView;
...
public class MainActivity extends DBFragmentActivity implements IDBFragmentConstants, ISettingConstants, IMusicConstant {
	...
	private void setUpPlayMusicLayout() {
		...
		ImageView mImgBg = (ImageView) mLayoutPlayMusic.findViewById(R.id.img_bg);
		...
		boolean b= IMyMusicPlayerConstants.USE_BACKGROUND;
		if(b){
			mImgBg.setImageResource(R.drawable.b1);
		}
		else{
			mImgBg.setBackgroundColor(Color.WHITE);
		}
	...
	private void setUpLayoutAdmob() {
		...
		if (ApplicationUtils.isOnline(this)) {
			boolean b= IMyMusicPlayerConstants.SHOW_ADVERTISEMENT;
			if (b) {
				adView = new AdView(this);
				adView.setAdUnitId(IMyMusicPlayerConstants.ADMOB_ID_BANNER);
			...
			DBListExcuteAction.getInstance().queueAction(new IDBCallback() {
				...
				public void onAction() {
					...
					if (mFragmentSearch != null) {
						mViewPager.setCurrentItem(IMyMusicPlayerConstants.LIBRARY_INDEX, true);
	...
	private FragmentPlaylist getFragmentPlaylist() {
		if (mListFragments != null && mListFragments.size() > 0) {
			Fragment mFragment = mListFragments.get(IMyMusicPlayerConstants.PLAYLIST_INDEX);
	...
	private FragmentHome getFragmentHome() {
		if (mListFragments != null && mListFragments.size() > 0) {
			Fragment mFragment = mListFragments.get(IMyMusicPlayerConstants.LIBRARY_INDEX);
	...
	private void createTab() {
		...
		mViewPager.setCurrentItem(IMyMusicPlayerConstants.LIBRARY_INDEX, false);
	...
	private void showDiaglogAboutUs() {
		...
		mBuilder.itemsCallback(new MaterialDialog.ListCallback() {
			...
			public void onSelection(MaterialDialog dialog, View itemView, int which, CharSequence text) {
				if (which == 0) {
					ShareActionUtils.shareViaEmail(MainActivity.this, IMyMusicPlayerConstants.YOUR_EMAIL_CONTACT, "", "");
				}
				else if (which == 1) {
					Intent mIntent = new Intent(MainActivity.this, ShowUrlActivity.class);
					mIntent.putExtra(KEY_URL, IMyMusicPlayerConstants.URL_YOUR_WEBSITE);
					mIntent.putExtra(IMyMusicPlayerConstants.KEY_HEADER, getString(R.string.title_website));
					startActivity(mIntent);
				}
				else if (which == 2) {
					Intent mIntent = new Intent(MainActivity.this, ShowUrlActivity.class);
					mIntent.putExtra(KEY_URL, IMyMusicPlayerConstants.URL_YOUR_FACE_BOOK);
					mIntent.putExtra(IMyMusicPlayerConstants.KEY_HEADER, getString(R.string.title_facebook));
					startActivity(mIntent);
				}
				else if (which == 3) {
					String url = String.format(IMyMusicPlayerConstants.URL_FORMAT_LINK_APP, getPackageName());
